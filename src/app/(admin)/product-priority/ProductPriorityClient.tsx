...
export default function ProductPriorityClient() {
  ...
  const loadProductPriorities = async () => {
    setLoading(true);
    try {
      const data = await ProductPriorityService.getProductPriorities({
        page,
        limit,
        search,
      });
      ...
      console.error(
        "Failed loading data rows from product_priority matrix:",
        err,
      );
    } finally {
      setLoading(false);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      await ProductPriorityService.createProductPriority(
        newPriorityName.trim(),
        newPriorityCode.trim(),
      );
      setNewPriorityName("");
      setNewPriorityCode("");
      setPage(1);
  ...
  const handleSaveUpdate = async (id: number) => {
    ...
    try {
      await ProductPriorityService.updateProductPriority(
        id,
        editingName.trim(),
        editingCode.trim(),
      );
      setProductPriorities(
        productPriorities.map((p) =>
          p.id === id
            ? { ...p, name: editingName.trim(), code: editingCode.trim() }
            : p,
        ),
      );
      setEditingId(null);
  ...
  const handleToggleDelete = async (id: number, isDeleted: boolean) => {
    ...
    try {
      const updated = await ProductPriorityService.toggleSoftDelete(
        id,
        isDeleted,
      );
      setProductPriorities(
        productPriorities.map((p) => (p.id === id ? updated : p)),
      );
    } catch (err) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-0.5">
              Configure Priority Products & Banking Segments
            </p>
          </div>
        </div>
                ...
                {loading ? (
                  <tr>
                    <td colSpan={6} className="py-20 text-center">
                      <div className="flex flex-col items-center justify-center gap-2">
                        <Loader2
                          className="animate-spin text-slate-350"
                          size={20}
                        />
                  ...
                  productPriorities.map((item) => {
                    ...
                    return (
                      <tr
                        key={item.id}
                        className={`group transition-colors ${isDeleted ? "bg-slate-50/40 opacity-70" : "hover:bg-slate-50/30"}`}
                      >
                        <td className="px-6 py-4 whitespace-nowrap text-xs font-mono font-bold text-slate-450">
                          {item.id}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          {isEditing ? (
                            ...
                            <span
                              className={`text-xs font-black ${isDeleted ? "text-slate-450 line-through" : "text-slate-800"}`}
                            >
                              {item.name}
                            </span>
                          )}
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-md border tracking-wider ${
                              isDeleted
                          ...
                          <div className="flex items-center justify-end gap-1">
                            {isEditing ? (
                              ...
                              <>
                                ...
                                <button
                                  onClick={() =>
                                    handleToggleDelete(item.id, isDeleted)
                                  }
                  ...
                  <tr>
                    <td
                      colSpan={6}
                      className="p-16 text-center text-slate-450 font-bold text-xs uppercase tracking-wider italic"
                    >
                      No options found.
                    </td>
                  </tr>
