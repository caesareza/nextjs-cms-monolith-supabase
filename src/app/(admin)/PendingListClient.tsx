...
export default function PendingListClient() {
  ...
  const summaryMetrics = useMemo(() => {
    ...
    const totalVolume = articles.reduce((sum, a) => sum + (a.demand || 0), 0);
    return { total, overdue, totalVolume };
  }, [articles]);

  const handleCopyKeyword = (id: number, text: string) => {
  ...
  const getMetaValidation = (meta: string) => {
    ...
    if (len >= 120 && len <= 160)
      return {
        label: `Optimal (${len} ch)`,
        style: "bg-emerald-50 text-emerald-700 border-emerald-200",
      };
    if (len < 120)
      return {
        label: `Too Short (${len} ch)`,
        style: "bg-amber-50 text-amber-700 border-amber-250",
      };
    return {
      label: `Too Long (${len} ch)`,
      style: "bg-rose-50 text-rose-700 border-rose-200",
    };
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {!loading && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 animate-in fade-in slide-in-from-top-3 duration-300">
          {/* Card 1: Total Pending */}
          <div className="bg-white p-5 border border-slate-200/60 rounded-2xl shadow-3xs flex items-center gap-4 transition-all hover:shadow-2xs">
          ...
          <div className="bg-white p-5 border border-slate-200/60 rounded-2xl shadow-3xs flex items-center gap-4 transition-all hover:shadow-2xs">
            ...
            <div>
              ...
              <p className="text-xl font-extrabold text-rose-600 mt-0.5 tabular-nums">
                {summaryMetrics.overdue}{" "}
                <span className="text-xs text-rose-455 font-semibold font-sans">
                  ASAP
                </span>
              </p>
            </div>
          ...
          <div className="bg-white p-5 border border-slate-200/60 rounded-2xl shadow-3xs flex items-center gap-4 transition-all hover:shadow-2xs">
            ...
            <div>
              ...
              <p className="text-xl font-extrabold text-slate-900 mt-0.5 tabular-nums">
                {summaryMetrics.totalVolume.toLocaleString("id-ID")}{" "}
                <span className="text-xs text-slate-400 font-semibold font-sans">
                  Demand
                </span>
              </p>
      ...
      {/* 2. THE MAIN ARTICLE QUEUE CONTAINER */}
                ...
                <>
                  ...
                  {segments.map((segment) => {
                    const count = articles.filter(
                      (a) =>
                        segment === "ALL" ||
                        a.product_priority?.name === segment,
                    ).length;
                    return (
                      <button
                        type="button"
