...
export default function CategoryClient() {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("Are you sure you want to safely archive this Category?"))
      return;
    try {
  ...
  const getBadgeStyle = (name: string) => {
    if (name.startsWith("[I]"))
      return "bg-[#1D4ED8]/10 text-[#1D4ED8] border-[#1D4ED8]/20"; // Individu blue-teal hue
    if (name.startsWith("[U]"))
      return "bg-amber-50 text-amber-700 border-amber-200"; // UKM Yellow hue
    if (name.startsWith("[K]")) return "bg-sky-50 text-sky-700 border-sky-200"; // Korporasi Light Blue
    if (name.startsWith("[S]"))
      return "bg-purple-50 text-purple-700 border-purple-200"; // Syariah Purple
    if (name.startsWith("OCBC") || name.endsWith("Banking"))
      return "bg-orange-50 text-orange-700 border-orange-200"; // Channels
  ...
  return (
    <div className="space-y-8">
      {" "}
      {/* Enforced Client Container Spacing Rule */}
      {/* BRAND HEADER */}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-0.5">
              Configure Segment Sub-Taxonomies & Channels
            </p>
          </div>
        </div>
        <div className="bg-slate-100 text-slate-650 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-wider w-fit">
      ...
      <form
        ...
        className="bg-white p-6 border border-slate-200 rounded-2xl shadow-sm flex gap-3 items-center"
      >
        <input
          type="text"
        ...
        <button
          type="submit"
          className="px-6 py-3 bg-slate-900 hover:bg-slate-800 text-white font-black text-xs uppercase tracking-wider rounded-xl transition-all flex items-center gap-2 shrink-0"
        >
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/50 text-[10px] font-black uppercase tracking-widest text-slate-400 select-none">
                <th className="px-8 py-5 font-black flex items-center gap-2">
                  <Hash size={12} /> ID
                </th>
                <th className="px-6 py-5 font-black">Sub-Taxonomy Node Tag</th>
                <th className="px-6 py-5 font-black hidden md:table-cell">
                  <Activity size={12} className="inline mr-1" /> Visibility
                </th>
                <th className="px-6 py-5 font-black hidden sm:table-cell">
                  <Calendar size={12} className="inline mr-1" /> Created Date
                </th>
              ...
              <tbody className="divide-y divide-slate-100">
                  ...
                  >
                    ...
                    <td className="px-6 py-4.5">
                      {editingId === category.id ? (
                        ...
                        <span
                          className={`px-3 py-1 border rounded-xl text-xs font-bold tracking-wide shadow-sm ${getBadgeStyle(category.name)}`}
                        >
                          {category.name}
                        </span>
                      )}
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        {editingId === category.id ? (
                          <>
                            <button
                              type="button"
                              onClick={() => handleSaveUpdate(category.id)}
                              className="p-2 bg-emerald-50 text-emerald-600 rounded-xl border border-emerald-100 hover:bg-emerald-100 transition-colors"
                            >
                              <Check size={14} />
                            </button>
                            <button
                          ...
                          <>
                            <button
                              ...
                              className="p-2 text-slate-400 hover:text-slate-800 bg-slate-50 hover:bg-slate-100 border border-slate-200/40 sm:opacity-0 sm:group-hover:opacity-100 rounded-xl transition-all"
                            >
                              <Edit2 size={13} />
                            </button>
                            <button
                              ...
                              className="p-2 text-slate-400 hover:text-brand-accent bg-slate-50 hover:bg-brand-accent/10 border border-slate-200/40 sm:opacity-0 sm:group-hover:opacity-100 rounded-xl transition-all"
                            >
                              <Trash2 size={13} />
                            </button>
                          </>
                ...
                {categories.length === 0 && (
                  <tr>
                    <td
                      colSpan={5}
                      className="p-16 text-center text-xs font-medium italic text-slate-400"
                    >
