...
export default function ArticleHistory({ logs = [] }: { logs: any[] }) {
  ...
  // Display top 3 logs if not expanded, or all logs when expanded
  const INITIAL_VISIBLE_COUNT = 3;
  const hasMoreLogs = sortedLogs.length > INITIAL_VISIBLE_COUNT;
  const displayedLogs =
    isExpanded || !hasMoreLogs
      ? sortedLogs
      : sortedLogs.slice(0, INITIAL_VISIBLE_COUNT);

  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-6">
      {/* Header with Title, Metrics, and Section Toggle */}
      <div className="flex items-center justify-between border-b border-slate-50 pb-4 flex-wrap gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] font-black bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full border border-slate-200">
                {sortedLogs.length} {sortedLogs.length === 1 ? "event" : "events"}
              </span>
            </div>
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tight mt-0.5">
              Lifecycle workflow transitions & log records
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 select-none">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 hover:bg-slate-100 text-slate-600 hover:text-slate-900 border border-slate-200 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all shadow-3xs cursor-pointer"
            title={isSectionOpen ? "Collapse audit history" : "Expand audit history"}
          >
            <span>{isSectionOpen ? "Hide" : "Show"}</span>
            {isSectionOpen ? <ChevronUp size={13} /> : <ChevronDown size={13} />}
          </button>
        </div>
      </div>
      ...
      {isSectionOpen && (
        <div className="space-y-6 animate-in fade-in duration-200">
          ...
          {/* Show More / Show Less Toggle Button when logs exceed threshold */}
          {hasMoreLogs && (
            <div className="pt-2">
              <button
                type="button"
                onClick={() => setIsExpanded(!isExpanded)}
                className="w-full py-3 px-4 bg-slate-50 hover:bg-slate-100 border border-slate-200/80 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-700 hover:text-brand-navy flex items-center justify-center gap-2 transition-all shadow-3xs hover:shadow-2xs cursor-pointer"
              >
                {isExpanded ? (
                  <>
                    <ChevronUp size={14} className="text-brand-accent" />
                    <span>Show Recent {INITIAL_VISIBLE_COUNT} Entries Only</span>
                  </>
                ) : (
                  <>
                    <ChevronDown size={14} className="text-brand-accent" />
                    <span>
                      View All {sortedLogs.length} History Logs (+{sortedLogs.length - INITIAL_VISIBLE_COUNT} more)
                    </span>
                  </>
                )}
              </button>
