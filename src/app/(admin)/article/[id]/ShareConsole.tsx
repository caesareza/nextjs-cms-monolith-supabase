...
export default function ShareConsole({ articleId, initialShareToken, initialShareActive }: ShareConsoleProps) {
    ...
    const handleToggleActive = async () => {
        ...
        try {
            ...
            const updated = await ArticleService.toggleShareActive(articleId, nextActive);
    ...
    const handleRegenerate = async () => {
        if (!confirm("Are you sure you want to regenerate the share token? The current preview link will immediately stop working.")) {
            return;
        }
        setUpdating(true);
        try {
    ...
    return (
        <div className="flex items-center gap-2 select-none">
            ...
            >
                <span className={`w-2 h-2 rounded-full ${shareActive ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500'}`} />
            ...
            >
                {copied ? 'Copied!' : <><Link2 size={13} /> Copy Link</>}
            </button>
