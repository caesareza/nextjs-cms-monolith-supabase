...
export default function ThemeClient() {
  ...
  const handleDelete = async (id: number) => {
    if (
      !confirm("Are you sure you want to permanently delete this Theme option?")
    )
      return;
    try {
      ...
      alert(
        "Cannot delete: This theme parameter is currently linked to active items.",
      );
    }
  };
  ...
  return (
    <div className="space-y-8">
      {" "}
      {/* Enforced Client Container Rule */}
      {/* HEADER METRICS */}
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-0.5">
            Configure Campaign Themes & Content Topics
          </p>
        </div>
      </div>
      {/* INPUT PANEL TRIGGER */}
        ...
        {loading ? (
          ...
          <div className="divide-y divide-slate-100">
            {themes.map((theme) => (
              <div
                key={theme.id}
                className="px-8 py-4 flex items-center justify-between group hover:bg-slate-50/50 transition-colors"
              >
                <div className="flex items-center gap-3">
                  {editingId === theme.id ? (
                    <input
                ...
                <div className="flex items-center gap-1.5">
                  {editingId === theme.id ? (
                    <>
                      <button
                        onClick={() => handleSaveUpdate(theme.id)}
                        className="p-2 bg-emerald-50 text-emerald-600 rounded-lg border border-emerald-100 hover:bg-emerald-100/60 transition-colors"
                      >
                        <Check size={14} />
                      </button>
                      <button
                        onClick={() => setEditingId(null)}
                        className="p-2 bg-slate-100 text-slate-500 rounded-lg hover:bg-slate-200/60 transition-colors"
                      >
                        <X size={14} />
                      </button>
                    </>
                    ...
                    <>
                      <button
                        ...
                        className="p-2 text-slate-400 hover:text-slate-700 bg-transparent rounded-lg opacity-0 group-hover:opacity-100 transition-all"
                      >
                        <Edit2 size={14} />
                      </button>
                      <button
                        onClick={() => handleDelete(theme.id)}
                        className="p-2 text-slate-400 hover:text-brand-accent bg-transparent rounded-lg opacity-0 group-hover:opacity-100 transition-all"
                      >
                        <Trash2 size={14} />
                      </button>
                    </>
