...
export default function PersonaClient() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPersonas(
        [...personas, created].sort((a, b) => a.name.localeCompare(b.name)),
      );
      setNewPersonaName("");
    } catch (err) {
  ...
  const handleDelete = async (id: number) => {
    if (
      !confirm(
        "Are you sure you want to safely archive this audience Persona parameter?",
      )
    )
      return;
    try {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-0.5">
              Configure Segment Personas & Target Profiles
            </p>
          </div>
        </div>
            ...
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/50 text-[10px] font-black uppercase tracking-widest text-slate-400 select-none">
                ...
                <th className="px-6 py-5 font-black">
                  Audience Profile Target Criteria
                </th>
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        {editingId === persona.id ? (
                          <>
                            <button
                              type="button"
                              onClick={() => handleSaveUpdate(persona.id)}
                  ...
                  <tr>
                    <td colSpan={5} className="p-16 text-center space-y-2">
                      <p className="text-xs font-medium italic text-slate-400">
                        Persona criteria database matrix currently holds zero
                        rows.
                      </p>
                    </td>
                  </tr>
