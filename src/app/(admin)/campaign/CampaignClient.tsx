...
export default function CampaignClient() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCampaigns(
        [...campaigns, created].sort((a, b) => a.name.localeCompare(b.name)),
      );
      setNewCampaignName("");
    } catch (err) {
  ...
  const handleDelete = async (id: number) => {
    if (
      !confirm(
        "Are you sure you want to archive this tracking Campaign target?",
      )
    )
      return;
    try {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-0.5">
              Configure Tracking Tags & Promo Key Attributions
            </p>
          </div>
        </div>
        <div className="bg-slate-100 text-slate-655 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-wider w-fit">
            ...
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/50 text-[10px] font-black uppercase tracking-widest text-slate-400 select-none">
                ...
                <th className="px-6 py-5 font-black">
                  Campaign Identifier / Tracking Tag
                </th>
                <th className="px-6 py-5 font-black hidden md:table-cell">
                  <Activity size={12} className="inline mr-1" /> Pipeline
                </th>
                <th className="px-6 py-5 font-black hidden sm:table-cell">
                  <Calendar size={12} className="inline mr-1" /> Date Formed
                </th>
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        {editingId === campaign.id ? (
                          <>
                            <button
                              type="button"
                              onClick={() => handleSaveUpdate(campaign.id)}
