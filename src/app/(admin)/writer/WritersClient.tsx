...
import { Loader2, UserPen, Hash, Calendar, Search } from "lucide-react";
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-0.5">
              Registered Editorial Content Authors
            </p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/50 text-[10px] font-black uppercase tracking-widest text-slate-400 select-none">
                ...
                <th className="px-8 py-5 font-black text-right hidden sm:table-cell">
                  <Calendar size={12} className="inline mr-1" /> Date Joined
                </th>
              </tr>
            </thead>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredWriters.map((writer, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4.5">
                      <span
                        className={`px-3 py-1 border rounded-xl text-xs font-bold tracking-wide shadow-sm whitespace-nowrap ${getBadgeStyle(index)}`}
                      >
                        {writer.name}
                      </span>
                    </td>
                ...
                {filteredWriters.length === 0 && (
                  <tr>
                    ...
                    >
                      {searchTerm
                        ? "No authors found matching search terms."
                        : "Writer lookup database matrix contains zero tracking rows."}
                    </td>
                  </tr>
                )}
              </tbody>
