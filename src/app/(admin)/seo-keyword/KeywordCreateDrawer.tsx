...
export default function KeywordCreateDrawer({
  isOpen,
  onClose,
  onSubmit,
  options,
}: KeywordCreateDrawerProps) {
  ...
  const jobCode = useMemo(() => {
    ...
    if (form.product_priority_id) {
      const priority = (options.productPriorities || []).find(
  ...
  const handleClose = () => {
    if (isDirty) {
      if (confirm("Discard changes? You will lose any unsaved content.")) {
        onClose();
      }
    } else {
      onClose();
  ...
  const handleSubmitInternal = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmit({ ...form, job_code: jobCode });
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden flex justify-end animate-in fade-in duration-200">
      ...
      >
        {/* DRAW HEADER PANEL */}
        <div className="flex items-center justify-between border-b border-slate-200/60 bg-white px-8 py-5 shrink-0 z-10">
          <div>
            <h2 className="text-lg font-black text-slate-900 tracking-tight">
              New Strategy Brief
            </h2>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">
              Initialize core focus requirements
            </p>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8">
          ...
          <div className="bg-white p-6 border border-slate-200/70 rounded-2xl space-y-4 shadow-xs">
            <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
              <span className="text-[10px] font-black text-slate-900 uppercase tracking-widest flex items-center gap-1.5">
                <span className="w-1.5 h-3 bg-brand-accent rounded-xs" />{" "}
                Content Taxonomy Cluster
              </span>
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block flex items-center gap-1.5">
                <ShieldCheck size={11} /> Priority Product
              </label>
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, product_priority_id: e.target.value })
                }
                required
              >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block flex items-center gap-1.5">
                  <FolderKanban size={11} /> Category Cluster
                </label>
                <select
                  className="w-full px-4 py-3 bg-slate-50/70 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 outline-none cursor-pointer"
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block flex items-center gap-1.5">
                  <FolderKanban size={11} /> Section Taxonomy
                </label>
                <select
                  className="w-full px-4 py-3 bg-slate-50/70 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 outline-none cursor-pointer"
          ...
          <div className="bg-white p-6 border border-slate-200/70 rounded-2xl space-y-4 shadow-xs">
            <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
              <span className="text-[10px] font-black text-slate-900 uppercase tracking-widest flex items-center gap-1.5">
                <span className="w-1.5 h-3 bg-brand-accent rounded-xs" /> Market
                Alignment Matrix
              </span>
              <span className="text-[9px] font-bold text-slate-400 uppercase">
                Section 2 of 3
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block flex items-center gap-1.5">
                  <Sparkles size={11} /> Strategy Theme
                </label>
                <select
                  className="w-full px-4 py-3 bg-slate-50/70 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 outline-none cursor-pointer"
                  value={form.theme_id}
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block flex items-center gap-1.5">
                <Megaphone size={11} /> Campaign Context
              </label>
              <select
                className="w-full px-4 py-3 bg-slate-50/70 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 outline-none cursor-pointer"
          ...
          <div className="bg-white p-6 border border-slate-200/70 rounded-2xl space-y-5 shadow-xs">
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block flex items-center gap-1.5">
                <ShieldCheck size={11} className="text-brand-accent" /> Content
            ...
            <div className="grid grid-cols-3 gap-3 bg-slate-50 p-4 border border-slate-200/60 rounded-2xl">
              <div className="space-y-1">
                <label className="text-[9px] font-black text-slate-500 uppercase tracking-wider flex items-center gap-1">
                  <Flame size={10} className="text-orange-500" /> Search Vol
                </label>
                <input
                  type="number"
                  className="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-800 outline-none focus:ring-2 focus:ring-brand-light-blue/30"
              ...
              <div className="space-y-1">
                ...
                <select
                  className="w-full px-2 py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-700 outline-none cursor-pointer"
                  value={form.intent}
            ...
            <div className="space-y-4 pt-2">
              <div className="space-y-1.5">
                ...
                <textarea
                  rows={2}
                  className="w-full px-4 py-3 bg-slate-50/70 border border-slate-200 rounded-xl text-xs font-bold text-slate-800 outline-none focus:bg-white resize-none transition-all placeholder:text-slate-300"
                  placeholder="Primary deep intent search phrase..."
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  rows={2}
                  className="w-full px-4 py-3 bg-slate-50/70 border border-slate-200 rounded-xl text-xs font-bold text-slate-800 outline-none focus:bg-white resize-none transition-all placeholder:text-slate-300"
                  placeholder="Separate related terms with commas (e.g. Kupon SR025, Beli SR025)..."
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between items-center">
                  ...
                  >
                    ...
                    {metaCharCount > 160 && "(Over 160 chars)"}
                  </span>
                </div>
                <textarea
                  rows={2}
                  className="w-full px-4 py-3 bg-slate-50/70 border border-slate-200 rounded-xl text-xs font-medium text-slate-700 outline-none focus:bg-white resize-none transition-all placeholder:text-slate-300"
                  placeholder="Google snippet summary parameter descriptions..."
        ...
        <div className="bg-white border-t border-slate-200/80 p-5 flex gap-3 shadow-[0_-8px_30px_rgb(0,0,0,0.04)] shrink-0 z-10">
          ...
          <button
            type="submit"
            disabled={
              submitLoading || !form.title.trim() || !form.target_keyword.trim()
            }
