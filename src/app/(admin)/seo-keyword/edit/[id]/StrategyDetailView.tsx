...
export default function StrategyDetailView({
  ...
  const computedJobCode = useMemo(() => {
    ...
    form.product_priority_id,
    form.content_type,
    form.target_keyword,
    lookups.productPriorities,
  ]);
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "seo_suite" | "assets" | "history"
  >("overview");
  ...
  useEffect(() => {
    ...
    const fetchAssets = async () => {
      ...
      try {
        const data = await ArticleService.getMarketingAssets(
          Number(dbSnapshot.id),
        );
  ...
  const cat =
    lookups.categories.find((c) => c.id === Number(form.category_id))?.name ||
    "Unspecified";
  const sec =
    lookups.sections.find((s) => s.id === Number(form.section_id))?.name ||
    "Unspecified";
  const prod =
    lookups.productTags.find((t) => t.id === Number(form.product_id))?.name ||
    "Standard Product";
  const priorityProd =
    (lookups.productPriorities || []).find(
      (p) => p.id === Number(form.product_priority_id),
    )?.name || "General";
  const thm =
    lookups.themes.find((t) => t.id === Number(form.theme_id))?.name ||
    "General Campaign Theme";
  const per =
    lookups.personas.find((p) => p.id === Number(form.persona_id))?.name ||
    "All Target Profiles";
  const cmp =
    lookups.campaigns.find((c) => c.id === Number(form.campaign_id))?.name ||
    "Organic Strategy";
  const titleCharCount = form.title.length;
  const titleWordCount =
    form.title.trim() === "" ? 0 : form.title.trim().split(/\s+/).length;
  const metaCharCount = form.meta_description.length;
  const metaWordCount =
    form.meta_description.trim() === ""
      ? 0
      : form.meta_description.trim().split(/\s+/).length;
    ...
    <div className="space-y-8 text-slate-900">
      ...
      {isApproved && (
        <div className="bg-emerald-50 text-emerald-800 border border-emerald-250/70 rounded-2xl p-4 flex items-center gap-3 text-xs font-semibold shadow-3xs">
          ...
          <span>
            This focus architecture is approved. Saving any edits below will
            instantly reset the gate to pending for Director re-review.
          </span>
        </div>
      )}
      ...
      <div className="space-y-4">
        <div className="space-y-1.5">
          <div className="flex justify-between items-center">
            ...
            {isEditing && (
              <span
                className={`text-[9px] font-bold ${titleCharCount > 60 ? "text-amber-600 font-extrabold" : "text-slate-400"}`}
              >
                {titleCharCount} chars / {titleWordCount} words{" "}
                {titleCharCount > 60 && "(Over 60 chars)"}
              </span>
            )}
          </div>
          {isEditing ? (
        ...
        <div className="flex flex-wrap items-center gap-x-6 gap-y-3 text-xs border-t border-slate-100 pt-3">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-1 font-bold text-slate-800">
              <span
                className={`w-2 h-2 rounded-full inline-block ${isApproved ? "bg-emerald-500" : "bg-amber-500"}`}
              />
              <span className="text-[11px] font-semibold">
                {isApproved ? "Active Strategy" : "Pending Review"}
              </span>
            </div>
          </div>
        ...
        <div className="flex gap-6 -mb-px">
          {[
            { id: "overview", name: "Overview", count: null },
            {
              ...
              count: marketingAssets.length || null,
            },
            {
              id: "history",
          ...
          <div className="space-y-8 animate-in fade-in duration-200">
            ...
            <div className="bg-white border border-slate-200/60 rounded-2xl p-8 shadow-3xs space-y-8">
              <h3 className="text-xs font-black text-slate-800 uppercase tracking-widest flex items-center gap-2 border-b border-slate-100 pb-3">
                <FileText size={13} className="text-slate-400" /> Basic
                Information
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6">
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block flex items-center gap-1.5">
                    <FolderKanban size={11} className="text-slate-350" />{" "}
                    Category / Cluster
                  </label>
                  {isEditing ? (
                    <select
                      className="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 outline-none cursor-pointer focus:bg-white focus:border-slate-450 transition-all mt-1"
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block flex items-center gap-1.5">
                    <Tag size={11} className="text-slate-350" /> Associated
                    Product
                  </label>
                  {isEditing ? (
                    <select
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block flex items-center gap-1.5">
                    <ShieldCheck size={11} className="text-slate-350" />{" "}
                    Priority Product
                  </label>
                  {isEditing ? (
                    <select
                      ...
                      onChange={(e) =>
                        setForm({
                          ...form,
                          product_priority_id: e.target.value,
                        })
                      }
                    >
                      <option value="">Select Priority Product...</option>
                      {(lookups.productPriorities || []).map((p) => (
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block flex items-center gap-1.5">
                    <Sparkles size={11} className="text-slate-350" /> Content
                    Theme
                  </label>
                  {isEditing ? (
                    <select
                      className="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 outline-none focus:bg-white focus:border-slate-450 transition-all mt-1"
                      value={form.theme_id}
                      onChange={(e) =>
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block flex items-center gap-1.5">
                    <Target size={11} className="text-slate-350" /> Target
                    Persona
                  </label>
                  {isEditing ? (
                    <select
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block flex items-center gap-1.5">
                    <Calendar size={11} className="text-slate-350" /> Target
                    Month
                  </label>
                  {isEditing ? (
                    <input
                      type="date"
                      className="w-full px-4 py-2 bg-slate-50/50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 outline-none focus:bg-white focus:border-slate-450 transition-all mt-1 cursor-pointer"
                      value={form.production_month}
                      onChange={(e) =>
                        setForm({ ...form, production_month: e.target.value })
                      }
                    />
                  ) : (
                    <p className="text-sm font-bold text-slate-800 mt-1 pl-[17px]">
              ...
              <div className="space-y-1 pt-4 border-t border-slate-100/60">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block flex items-center gap-1.5">
                  <Megaphone size={11} className="text-slate-350" /> Campaign
                  Context
                </label>
                {isEditing ? (
                  <select
                ...
                <div className="flex-1 min-w-0">
                  {isEditing ? (
                    <div className="space-y-0.5">
                      ...
                      <input
                        type="number"
                        className="w-full px-2 py-1 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold text-slate-800 outline-none"
                        value={form.demand}
                        onChange={(e) =>
                    ...
                    <>
                      ...
                      <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-1 truncate">
                        Search Demand
                      </div>
                    </>
                  )}
                ...
                <div className="flex-1 min-w-0">
                  {isEditing ? (
                    <div className="space-y-0.5">
                      ...
                      <select
                        className="w-full px-1.5 py-1 bg-slate-50 border border-slate-200 rounded-lg text-[10px] font-bold text-slate-800 outline-none cursor-pointer"
                        value={form.intent}
                        onChange={(e) =>
            ...
            <div className="bg-white p-8 border border-slate-200/60 rounded-2xl shadow-3xs space-y-6">
              <h3 className="text-xs font-black text-slate-800 uppercase tracking-widest flex items-center gap-2 border-b border-slate-100 pb-3">
                <FileText size={13} className="text-slate-400" /> SEO Copy Suite
                Validation
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="space-y-2 md:col-span-1">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block flex items-center gap-1.5">
                    <FileText size={11} className="text-slate-350" /> Target
                    Focus Keyword
                  </span>
                  {isEditing ? (
                    <textarea
                      rows={3}
                      className="w-full text-xs font-mono font-black text-slate-900 bg-slate-50/50 border border-slate-200 rounded-xl p-3 resize-none outline-none focus:bg-white focus:border-slate-400 transition-all"
                ...
                <div className="space-y-2 md:col-span-1">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block flex items-center gap-1.5">
                    <FileText size={11} className="text-slate-350" /> Related
                    Keywords
                  </span>
                  {isEditing ? (
                    <textarea
                      rows={3}
                      className="w-full text-xs font-bold text-slate-800 bg-slate-50/50 border border-slate-200 rounded-xl p-3 resize-none outline-none focus:bg-white focus:border-slate-400 transition-all placeholder:text-slate-300"
                      placeholder="Separate keywords with commas..."
                      value={form.related_keyword}
                      onChange={(e) =>
                        setForm({ ...form, related_keyword: e.target.value })
                      }
                    />
                  ) : (
                ...
                <div className="space-y-2 md:col-span-1">
                  <div className="flex justify-between items-center">
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block flex items-center gap-1.5">
                      <FileText size={11} className="text-slate-350" /> Meta
                      Description Tag
                    </span>
                    {isEditing && (
                      <span
                        className={`text-[9px] font-bold ${(metaCharCount < 120 || metaCharCount > 160) && metaCharCount > 0 ? "text-amber-600 font-extrabold" : "text-slate-400"}`}
                      >
                        {metaCharCount} chars / {metaWordCount} words{" "}
                        {metaCharCount > 0 && metaCharCount < 120 && "(Short)"}{" "}
                        {metaCharCount > 160 && "(Over)"}
                      </span>
                    )}
                  </div>
                  {isEditing ? (
                    <textarea
                      rows={3}
                      className="w-full text-xs font-bold text-slate-800 bg-slate-50/50 border border-slate-200 rounded-xl p-3 resize-none outline-none focus:bg-white focus:border-slate-400 transition-all"
                ...
                <div className="space-y-2 md:col-span-3 pt-4 border-t border-slate-100">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block flex items-center gap-1.5">
                    <Link2 size={11} className="text-slate-350" /> Internal
                    Reference Link Maps
                  </span>
                  {isEditing ? (
                    <textarea
                      rows={2}
                      className="w-full text-xs font-mono text-slate-600 bg-slate-50/50 border border-slate-200 rounded-xl p-3 resize-none outline-none focus:bg-white focus:border-slate-400 transition-all"
                    ...
                    <div className="w-full text-xs font-mono font-semibold text-slate-650 bg-slate-50/70 border border-slate-200/60 rounded-xl p-4 min-h-14 leading-relaxed break-all select-all">
                      {form.cta_internal_link ||
                        "No dynamic internal reference paths generated."}
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="bg-white p-8 border border-slate-200/60 rounded-2xl shadow-3xs space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-3">
                  <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest border-b border-slate-100 pb-2 flex items-center gap-1.5">
                    <Link2 size={13} className="text-slate-400" /> Call To
                    Actions (CTAs)
                  </h4>
                    ...
                    {loadingAssets ? (
                      ...
                      marketingAssets
                        ...
                        .map((a, i) => (
                          <div
                            key={a.id}
                            className="text-xs bg-slate-50 border border-slate-100 rounded-2xl p-4 shadow-2xs hover:border-slate-200 transition-colors"
                          >
                ...
                <div className="space-y-3">
                  <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest border-b border-slate-100 pb-2 flex items-center gap-1.5">
                    <Package size={13} className="text-slate-400" /> Recommended
                    Products
                  </h4>
                  <div className="space-y-2.5">
                    {loadingAssets ? (
              ...
              <div className="border-t border-slate-100/80 pt-6">
                <form
                  ...
                  className="bg-slate-50 border border-slate-200/80 rounded-2xl p-5 space-y-4 shadow-3xs"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200/50 pb-4">
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 font-bold mt-0.5">
                        Quick-log a CTA url or a Product recommendation card
                      </p>
                    </div>
                  ...
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      placeholder={
                        newAssetType === "cta"
                          ? "Enter asset URL destination link (e.g. https://www.ocbc.id/...)"
                          : "Enter product slug or recommendation tag (e.g. tabungan)"
                      }
        ...
        {activeTab === "history" && (
          <div className="space-y-6 animate-in fade-in duration-200">
            {dbSnapshot?.internal_notes &&
              dbSnapshot.internal_notes.trim() !== "" && (
